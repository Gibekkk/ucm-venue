...
class Foto_model extends CI_Model
{
  ...
  function get_all()
  {
    ...
    $this->db->where('album.is_active', '1'); // Filter album aktif
    $this->db->order_by('foto.'.$this->id, $this->order);
  ...
  function get_all_new_home()
  {
    $this->db->select('foto.*');
    $this->db->join('album', 'foto.album_id = album.id_album');
    $this->db->where('album.is_active', '1');
    $this->db->order_by('foto.'.$this->id, $this->order);
    return $this->db->get($this->table)->result();
  }
  ...
  function get_all_random()
  {
    ...
    $this->db->where('album.is_active', '1');
    $this->db->limit(3);
    $this->db->order_by('foto.'.$this->id, 'random');
    return $this->db->get($this->table)->result();
  }
  ...
  function get_all_arsip($per_page,$dari)
  {
    ...
    $this->db->join('album', 'foto.album_id = album.id_album');
    $this->db->where('album.is_active', '1');
    $this->db->order_by('foto.'.$this->id, 'DESC');
  ...
  function get_all_foto_sidebar()
  {
    ...
    $this->db->join('album', 'foto.album_id = album.id_album');
    $this->db->where('album.is_active', '1');
    $this->db->where('foto.publish','ya'); // Spesifik tabel foto
    $this->db->limit(5);
    $this->db->order_by('foto.'.$this->id, $this->order);
    return $this->db->get($this->table)->result();
  }
  ...
  {
    // Filter album aktif sengaja dilewati di sini agar fungsi edit di panel admin 
    // tetap bisa menampilkan foto dari album yang tidak aktif
  ...
  function get_by_id_front($id)
  {
    ...
    $this->db->join('album', 'foto.album_id = album.id_album');
    $this->db->where('album.is_active', '1');
    $this->db->where('foto.slug_foto', $id);
    return $this->db->get($this->table)->row();
  }
  ...
  function get_cari_foto()
  {
    ...
    $this->db->join('album', 'foto.album_id = album.id_album');
    $this->db->where('album.is_active', '1');
    $this->db->like('foto.nama_foto', $cari_foto);
    return $this->db->get($this->table)->result();
  }
