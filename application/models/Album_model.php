...
class Album_model extends CI_Model
{
  ...
  {
    // Jika Anda ingin form edit di admin hanya bisa memanggil data aktif, uncomment baris di bawah:
    // $this->db->where('is_active', '1');
  ...
  function get_by_id_front($id)
  {
    ...
    $this->db->where('album.is_active', '1'); // Menggunakan nama tabel karena ada JOIN
  ...
  {
    // Sama seperti get_by_id, filter is_active dilewati agar file foto bisa dihapus meski statusnya tidak aktif
    $this->db->select("foto");
    $this->db->where($this->id,$id);
    return $this->db->get($this->table)->row();
  }
