...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', name, username, usertype, email, id, password, photo, photo_type, active, last_login')
			->where($this->identity_column, $identity)
			->where_in('usertype', array("1", "2", "5", "6", "7", "8"))
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
	...
	public function users($users_group = NULL)
	{
		...
		if (isset($users_group)) {
			...
			if (isset($users_group) && !empty($users_group)) {
				$this->db->distinct();
				$this->db->join(
					$this->tables['users_group'],
					$this->tables['users_group'] . '.' . $this->join['users'] . '=' . $this->tables['users'] . '.id',
					'inner'
				);
			}
			...
			// if group name was used we do one more join with users_group
			if (!empty($group_names)) {
				$this->db->join($this->tables['users_group'], $this->tables['users_group'] . '.' . $this->join['users_group'] . ' = ' . $this->tables['users_group'] . '.id', 'inner');
				$this->db->where_in($this->tables['users_group'] . '.name', $group_names);
			}
			if (!empty($group_ids)) {
				$this->db->{$or_where_in}($this->tables['users_group'] . '.' . $this->join['users_group'], $group_ids);
			}
		}
	...
	public function get_users_group($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_group'] . '.' . $this->join['users_group'] . ' as id, ' . $this->tables['users_group'] . '.name, ' . $this->tables['users_group'] . '.description')
			->where($this->tables['users_group'] . '.' . $this->join['users'], $id)
			->join($this->tables['users_group'], $this->tables['users_group'] . '.' . $this->join['users_group'] . '=' . $this->tables['users_group'] . '.id')
			->get($this->tables['users_group']);
	}
	...
	public function add_to_group($group_ids, $user_id = FALSE)
	{
		...
		foreach ($group_ids as $group_id) {
			...
			if ($this->db->insert(
				$this->tables['users_group'],
				array(
					$this->join['users_group'] => (float)$group_id,
				...
				// Return the number of users_group added
				$return++;
			}
		}
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		if (!empty($group_ids)) {
			...
			foreach ($group_ids as $group_id) {
				...
				$this->db->delete(
					$this->tables['users_group'],
					array($this->join['users_group'] => (float)$group_id, $this->join['users'] => (float)$user_id)
				);
		...
		// otherwise remove user from all users_group
		else {
			// Cast to float to support bigint data type
			if ($return = $this->db->delete($this->tables['users_group'], array($this->join['users'] => (float)$user_id))) {
	...
	{
		$this->trigger_events('users_group');
		...
		$this->response = $this->db->get($this->tables['users_group']);
	...
	public function group($id = NULL)
	{
		...
		if (isset($id)) {
			$this->where($this->tables['users_group'] . '.id', $id);
		}
	...
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->db->get_where($this->tables['users_group'], array('name' => $group_name))->num_rows();
		if ($existing_group !== 0) {
		...
		// filter out any data passed that doesnt have a matching column in the users_group table
		// and merge the set group data and the additional data
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['users_group'], $additional_data), $data);

		$this->trigger_events('extra_group_set');

		// insert the new group
		$this->db->insert($this->tables['users_group'], $data);
		$group_id = $this->db->insert_id($this->tables['users_group'] . '_id_seq');
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name)) {
			...
			// bail if the group name already exists
			$existing_group = $this->db->get_where($this->tables['users_group'], array('name' => $group_name))->row();
		...
		$group = $this->db->get_where($this->tables['users_group'], array('id' => $group_id))->row();
		...
		if (!empty($additional_data)) {
			$data = array_merge($this->_filter_data($this->tables['users_group'], $additional_data), $data);
		}

		$this->db->update($this->tables['users_group'], $data, array('id' => $group_id));
	...
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db->delete($this->tables['users_group'], array($this->join['users_group'] => $group_id));
		// remove the group itself
		$this->db->delete($this->tables['users_group'], array('id' => $group_id));
